...
## How to know the project works

Run the readiness check against the running app:
...
The script fails if:
- `/api/jobs` has an `error`
- `jobs` is not an array
- fewer than `MIN_JOBS` jobs are returned (default: `1`)

Configurable options:

```bash
BASE_URL="http://localhost:3000" MIN_JOBS=1 ./scripts/verify-readiness.sh
```

For first-time smoke tests when you expect zero jobs, you can set `MIN_JOBS=0`.
